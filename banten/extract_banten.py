#!/usr/bin/env python3
"""Ekstraksi subset GIS Banten (read-only terhadap sumber).

Menyalin VERBATIM fitur yang lolos filter dari ``data/`` ke folder output,
tanpa menyentuh file asli (dibuka hanya mode ``"rb"``, (size, mtime_ns)
diverifikasi sebelum & sesudah, sha256 sumber dihitung dalam pass yang sama).

Pola & penjaga keselamatan mengikuti ``data/bekasi-cikarang/extract_bekasi_cikarang.py``
(yang memakai filter per-kabupaten). Skrip ini memakai filter **per-provinsi**:
seluruh Kabupaten/Kota di Provinsi Banten (kode Kemendagri ``36``).

Contoh:
    python extract_banten.py
    python extract_banten.py --only kecamatan --only roads
    python extract_banten.py --out "D:/Berijalan/repo/geoservice/data/banten"
"""
...
PROVINSI_CODE = "36"                 # Banten
PROVINSI_NAME = "Banten"             # roads: atribusi hanya lewat nama
OUT_SUFFIX = "banten"

# folder sumber: key, filename, jumlah fitur di header (penjaga integritas)
COLLECTIONS = (
...
# Jumlah hasil filter yang SUDAH DIUKUR pada dataset ini (grep, C-speed).
# Bukan estimasi: kalau predicate berubah/rusak, guard ini gagal keras.
EXPECTED_MATCH = {
    "provinsi": 8,            # 36.01 36.02 36.03 36.04 36.71 36.72 36.73 36.74
    "kabupaten_kota": 8,      # idem
    "kecamatan": 155,
    "desa_kelurahan": 1516,
    "roads": 20414,
}
...
def match(kind: str, f: dict) -> bool:
    """True bila fitur *f* berada di Provinsi Banten.

    Kecamatan/desa memakai uji jumlah titik pada kode (2 = kecamatan,
    3 = desa) supaya baris placeholder ber-``kode`` tingkat kabupaten
    (mis. ``52.01``) tidak ikut lolos.
    """
    if kind in ("provinsi", "kabupaten_kota"):
        kk = f.get("kode_kemendagri") or {}
        return kk.get("provinsi") == PROVINSI_CODE and str(kk.get("kabupaten") or "").startswith(PROVINSI_CODE + ".")
    if kind == "kecamatan":
        kode = _kode(f, "kecamatan")
        return kode.count(".") == 2 and kode.startswith(PROVINSI_CODE + ".")
    if kind == "desa_kelurahan":
        kode = _kode(f, "desa")
        return kode.count(".") == 3 and kode.startswith(PROVINSI_CODE + ".")
    if kind == "roads":
        return f.get("provinsi") == PROVINSI_NAME
...
def extract_collection(kind: str, filename: str, expected: int, out_dir: Path) -> dict:
    src = DATA_DIR / filename
    out_path = out_dir / f"{kind}.{OUT_SUFFIX}.json"
    ...
    want = EXPECTED_MATCH.get(kind)
    if want is not None and matched != want:
        out_path.unlink(missing_ok=True)
        raise SystemExit(
            f"FATAL: {filename} menghasilkan {matched} fitur Banten, diharapkan {want}. "
            "Predicate berubah — output dibatalkan."
        )
    return {
...
def extract_roads(out_dir: Path) -> dict:
    kind, filename = ROADS
    src = DATA_DIR / filename
    out_path = out_dir / f"{kind}.{OUT_SUFFIX}.json"
    sha = hashlib.sha256()
    ...
    with open(src, "rb") as fh, open(out_path, "wb") as out:   # sumber READ-ONLY
        ...
        for lineno, line in enumerate(fh, 1):
            ...
            if not s.startswith(b"{"):
                ...
                continue
            feature_lines += 1
            if b"Banten" not in s:                              # prefilter murah
                continue
            parsed += 1
            try:
                obj = json.loads(s)
    ...
    want = EXPECTED_MATCH.get("roads")
    if want is not None and matched != want:
        out_path.unlink(missing_ok=True)
        raise SystemExit(
            f"FATAL: roads menghasilkan {matched} fitur Banten, diharapkan {want}. "
            "Predicate berubah — output dibatalkan."
        )
    return {
        "file": out_path.name, "features": matched,
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Ekstraksi subset GIS Banten (read-only).")
    ...
    print(f"Provinsi   : {PROVINSI_CODE} {PROVINSI_NAME}")
    ...
    for kind, filename, expected in COLLECTIONS:
        if kind in kinds:
            ...
            res = extract_collection(kind, filename, expected, out_dir)
    ...
    manifest = {
        ...
        "scope": f"provinsi-{PROVINSI_CODE}-{PROVINSI_NAME.lower()}",
        "predicate": {
            "provinsi_kode": f"{PROVINSI_CODE} {PROVINSI_NAME}",
            "kabupaten": "semua kabupaten/kota dengan kode_kemendagri.kabupaten berawalan '36.'",
            "catatan": "kecamatan/desa difilter lewat kode_kemendagri (jumlah titik 2/3); roads lewat nama provinsi",
